...
def _write_gold_instrument_metadata(gold: pl.DataFrame, lake_root: str, plot: bool = True) -> list[str]:
    ...
    for partition in gold.partition_by(["exchange"]):
        ...
        upsert_partition_parquet(
            file_path=file_path,
            partition=partition,
            natural_key=INSTRUMENT_METADATA_GOLD_NATURAL_KEY,
...
def _write_gold_instrument_metadata_plot(gold: pl.DataFrame, path: Path) -> None:
    ...
    write_compact_feature_profile_png(
        frame=gold,
        path=path,
        x_column="snapshot_date",
        y_columns=["instrument_count", "active_instrument_count", "option_instrument_count", "mean_strike"],
        color="#a3be8c",
        marker="o",
    )
