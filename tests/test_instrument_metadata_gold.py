...
from ingestion.instrument_metadata_gold import (
    _gold_instrument_metadata_from_silver,
    _write_gold_instrument_metadata,
...
def test_transform_instrument_metadata_silver_to_gold_writes_and_skips_unchanged(
    tmp_path: Path,
) -> None:
    """Verify Gold instrument-metadata transform writes state and skips unchanged inputs."""

    silver_root = tmp_path / "silver"
    silver_dir = (
        silver_root / "dataset_type=instrument_metadata_snapshot_features_daily" / "exchange=deribit" / "month=2026-06"
    )
    ...
    first_files = transform_instrument_metadata_silver_to_gold(
        silver_lake_root=str(silver_root),
        gold_lake_root=str(tmp_path / "gold"),
        plot=False,
    )
    second_files = transform_instrument_metadata_silver_to_gold(
        silver_lake_root=str(silver_root),
        gold_lake_root=str(tmp_path / "gold"),
        plot=False,
    )
...
def test_transform_instrument_metadata_silver_to_gold_rejects_unknown_fill_policy(
    tmp_path: Path,
) -> None:
    """Verify Gold instrument-metadata transform rejects unsupported fill policies."""
