...
def test_index_price_partition_path() -> None:
    result = index_price_partition_path(
        "lake/bronze",
        ("index_price_snapshot_1m", "deribit", "btc_usd", "2026-05", "2026-05-24"),
    )
    assert str(result).endswith(
        "dataset_type=index_price_snapshot_1m/exchange=deribit/index_name=btc_usd/month=2026-05/date=2026-05-24"
    )
